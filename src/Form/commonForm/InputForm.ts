...
import { LangText, translator, UniversalTranslator } from "../../Translate";
...
> {
    ...
    fieldsGenerator?: (player: Player, args: U, t: UniversalTranslator) => BaseField[];
...
> {
    ...
    builder(p: Player, args: U) {
        ...
        const fields = [...(data.fields ?? []), ...(data.fieldsGenerator?.(p, args, t) ?? [])];
