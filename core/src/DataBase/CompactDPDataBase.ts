 ...
 *
 * 每个字段编码为 `<payloadLength(base36)>:<payload>`，字段名与类型不写入数据。
 * 字符串内容可以包含任意分隔符，因为解码依赖长度而不是转义字符。
 */
    ...
    decode(input: unknown): CompactDecodeResult<CompactStructValue<TFields>> {
        ...
        for (let fieldIndex = 0; fieldIndex < this.fields.length; fieldIndex++) {
            ...
            if (offset === input.length) {
                ...
                result[name] = defaultResult.value;
                continue;
            }
...
/**
 * 只允许读写固定 schema 的结构化数据，并使用 DP 持久化。
 *
 * schema 的数组顺序就是持久化协议中的字段位置。已投入使用后不要重排或在中间插入字段；
 * 需要兼容旧记录时，只在末尾追加带 default 的字段。
 */
...
> extends DataBase<CompactStructValue<TFields>> {
    ...
    constructor(name: string, fields: TFields, source: DPSource = world) {
        ...
        this.codec = new CompactStructCodec(fields);
        ...
        // 对外应暴露只允许结构化数据的 CompactDPDataBase，而不是原始存储层。
...
function validateSchema(fields: readonly CompactFieldDefinition[]): void {
    ...
    fields.forEach((field, fieldIndex) => {
        ...
        if (field.length >= 3) {
            const options = field[2] as { default?: unknown } | undefined;
