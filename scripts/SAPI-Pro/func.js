...
export function cmd(text, async = true) {
    if (async) {
...
/**生成随机数∈[min,max] */
export function rand(min = 0, max) {
...
export function distance_sqrt(pos1, pos2) {
    return Math.sqrt(distance(pos1, pos2));
}
export const dimName = {
    "minecraft:overworld": "§q主世界",
    "minecraft:nether": "§m下界",
    "minecraft:the_end": "§n末地",
};
export function getScoreboardObj(scoreboardName) {
...
export function Vector3Add(vec1, vec2) {
...
export function LibMessage(text) {
    console.log(`[${libName}]${text}`);
}
export function LibError(text) {
