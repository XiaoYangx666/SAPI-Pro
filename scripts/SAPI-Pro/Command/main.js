...
export class Command {
     ...
     */
    constructor(name, explain, isAdmin, handler, validator, isHidden = false, isClient = false) {
        this.paramBranches = [];
        this.subCommands = [];
        this.name = name;
    ...
    static fromParamBranches(paramBranches) {
        let subParams = [];
...
export class ParseInfo {
    constructor(value, cnt = 1) {
        this.value = value;
        this.cnt = cnt;
    }
}
export class ParseError {
    constructor(msg, onlymsg, index = 0, canReplace = true) {
        this.msg = msg;
...
export class commandParser {
    ...
    /**客户端注册指令 */
    regToHost() {
        if (LibConfig.UUID == undefined)
            return;
        const obj = [...this.commands.entries()].reduce((obj, [key, value]) => ((obj[key] = value), obj), {});
    ...
    regClientCommand() {
        try {
            const commandObj = exchangedb.get("cmd");
    ...
    /** 运行命令注册回调*/
    runCommand(t) {
        if (!(t.message.length > 0 && t.message[0] === "."))
            return;
    ...
    parseCommand(input, player) {
        //使用正则分割字符串
        const paramStrings = [...input.matchAll(/@?(?:"(?:[^"]*)"|(?:[^\s]+))/g)].map((t) => t[0]);
    ...
    /**寻找最深的命令 */
    findCommand(command, paramStrings) {
        let current = 0;
    ...
    parseSubCommand(command, paramStrings, player) {
        //先拿到目标命令
    ...
    parseParams(command, subCommand, params, current, player) {
        ...
        if (subCommand.paramBranches.length == 0) {
            if (paramStrings.length != 0)
                return commandParser.ErrorMessage(player, command, paramStrings[0], params, current, "多余参数");
            return {};
        }
        //转换后参数对象
        const parsedParamDict = {};
        let paramError = { msg: "", index: 0, canReplace: true };
        let success = false;
        const setParamValue = (stack) => {
            ...
            for (let i = 0; i < stack.length; i++) {
                ...
                if (value != undefined)
                    parsedParamDict[param.name] = value;
            }
        };
        const updateError = (index, error, canReplace = false) => {
        ...
        const checkSuccess = (T, ctx, stack) => {
            const paramDef = T.param;
            ...
            if (checkResult === true) {
                ...
                if (checkResult.onlymsg) {
                    ...
                    updateError(index + checkResult.index, commandParser.BuildErrorMessage(command, paramStrings[index + checkResult.index], params, current + index + checkResult.index, checkResult.msg), checkResult.canReplace);
                }
                return traverseAct.back;
            }
        };
        ...
        for (const paramDef of subCommand.paramBranches) {
            if (!success)
                PreOrdertraverse({ param: paramDef, index: 0 }, {
                    getSubNodes: (T) => {
    ...
    static ErrorMessage(player, command, value, params, current, tip) {
        if (!testMode)
