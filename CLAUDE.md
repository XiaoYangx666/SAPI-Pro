...
- **每渠道类型单独安装**：`variants/beta` 与 `variants/stable` 各自的 `devDependencies` 钉各自的 `@minecraft/*`，需 `npm run install:variants`（两个目录各有 package-lock.json）。根目录的 `@minecraft/*` 仅用于测试/编辑器的 beta 类型。三者不得混用。
