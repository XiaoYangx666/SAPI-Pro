...
- `core/src/DataBase/`：`DPDataBase` / `CompactDPDataBase` / `ScoreBoardJSONDataBase` / `ScoreBoardDataBase`。`CompactDPDataBase` 用固定 schema 的位置编码紧凑存储同构 DP 记录；超大文本继续复用 DP 分片。
